...
void informalTests() {
  ...
  if (showWelcome) {
    ...
    std::cout << "Make sure to inspect informalTests() in main.cpp to turn on "
                 "the available"
              << std::endl
              << "informal tests. The terminal output for each test is "
                 "lengthy, so you may"
              << std::endl
              ...
              << "If the diagram plots in the terminal cause problems with "
                 "screen readers,"
              << std::endl
              << "then please adjust the verbosity settings in the "
              ...
              << "Those are defined in the file(s) in the tests/ subdirectory. "
                 "Since those"
              << std::endl
              << "other unit tests are separate from the main program, you "
                 "must compile and"
              << std::endl
  ...
  if (doEx3Tests) {
    ...
    std::cout << "Now running puzzleBFS tests. Some tests may be disabled for "
                 "speed; see"
              << std::endl
              << "informalTests() in main.cpp for details. If any tests take a "
                 "very long time,"
              << std::endl
              << "you probably have a bug. You can cancel the running process "
                 "in the terminal"
              << std::endl
    ...
    if (!doEx3SlowTests)
      std::cout << "(informalTests: doEx3SlowTests==false, so we'll skip some "
                   "slow tests.)"
                << std::endl
                << std::endl;

    if (doEx3SlowTests)
      puzzleBFS_Tester_NoPath();  // This one is slow even when your code is
                                  // correct!
  ...
  std::cout
      << "\nInformal tests finished without crashing, but the output may have "
         "been incorrect."
...
void demoRunner() {
  {
    ...
    std::cout
        << "In this project, we use std::unordered_set in several places to "
           "represent"
        << std::endl
        << "collections of items having some shared property. A set contains "
           "value copies"
        << std::endl
        << "of objects, and it contains no duplicates; inserting the same item "
           "twice has"
        << std::endl
        << "no effect. In C++, the unordered set type uses hashing internally, "
           "so the"
        << std::endl
        << "key type must support equality comparison and std::hash. Unlike "
           "the unordered"
        << std::endl
        << "map type, the keys for an unordered set do not have associated "
           "values. We only"
        << std::endl
        << "care whether a given key has been inserted or not." << std::endl
        << std::endl
        << "This is an alternative to \"labeling\" vertices with some status, "
           "for example."
        << std::endl
        << "If an algorithm tells you to label vertices as \"visited\", you "
           "could instead"
        << std::endl
        << "make a set for all the visited vertices, and insert copies of "
           "vertices to the"
        << std::endl
        << "visited set when you want to record that they have been visited."
        << std::endl
        << std::endl
        << "For example, we could make a set to contain unique integers like "
           "this:"
        << std::endl
        << std::endl
    ...
    std::cout << "Iterating over std::unordered_set contents. (These may not "
                 "be in any order.)"
              << std::endl;
    ...
    std::cout << "And then we can use count() to query if an item is in the "
                 "set or not, and we"
              << std::endl
    ...
    // operation. The order of evaluation for the stream syntax may not be what
    // you expect.
  ...
  {
    std::cout << makeHeader2("Undirected edge representation with IntPairPair")
              << std::endl
              << std::endl;

    std::cout << "Our GridGraph class doesn't explicitly store edges since it "
                 "uses an adjacency"
              << std::endl
              << "list scheme (actually, an adjacency map and set scheme in "
                 "our implementation)."
              << std::endl
              << "However, sometimes we may want to examine the undirected "
                 "edges of the graph"
              << std::endl
              << "as explcit items. For example, GridGraph::printDetails makes "
                 "a set of unique"
              << std::endl
              << "edges in order to display terminal output." << std::endl
              << std::endl;

    std::cout << "This is where we have to be careful, because IntPairPair is "
                 "just a pair of"
              << std::endl
    ...
    std::cout << "Creating IntPairPair AB from {pointA, pointB} and BA from "
                 "{pointB, pointA}."
              << std::endl;
    ...
    std::cout
        << "If you wanted to extend the code further, you could make a derived "
           "class"
        << std::endl
        << "based on IntPairPair that would ensure that edges (A,B) and (B,A) "
           "would"
        << std::endl
...

  {
    ...
    std::cout << "As you examine the provided code, you may see objects being "
                 "initialized in"
              << std::endl
              << "various ways. Remember that in C++11 and newer, there are "
                 "compiler features"
              << std::endl
              << "that can infer (detect) the correct STL types automatically, "
                 "which can help"
              << std::endl
              << "you write neater code." << std::endl
              << std::endl
              << "Our IntPair type is based on std::pair<int,int>. The lines "
                 "below all construct"
              << std::endl
              << "the same IntPair. The last one uses an \"initializer list\" "
                 "in braces {}, which"
              << std::endl
              << "can be used wherever the type can be understood correctly by "
                 "the compiler."
              << std::endl
              << std::endl
    ...
    std::cout << "Verifying equality:" << std::endl
              << std::boolalpha << "  point1 == point2 : " << (point1 == point2)
              << std::endl
    ...
    std::cout
        ...
        << std::endl
        << "The compiler needs to be able to tell if you mean IntPair, "
           "std::vector,"
        << std::endl
        << "or something else, based on the structure of the variable and how "
           "you use it."
        << std::endl
        << "The following may or may not work, depending on the context of "
           "your code:"
        << std::endl
        << std::endl
...
void removePointTester() {
  ...
  for (const auto& kv : graph.adjacencyMap) {
    ...
    if (k == IntPair(2, 2)) {
      ...
      // the value type is a set of neighboring points that are adjacent to this
      // point in the graph
  ...
  if (graph == edgelessGraph) {
    ...
    errorReaction(
        "The point removal should only remove the point\n and all the "
        "adjacency records that refer to it.");
  }
...
void graphBFS_Tester() {
  ...
  // beforehand.) Note that for convenience, we can use the C++11 "initializer"
  // syntax in curly braces {} to concisely enter all these nested objects. As
  // long as the compiler can find constructors that match the types we're
  // using, it will be okay.
  ...
  if (path.empty()) {
    errorReaction(
        "Could not reach goal from start. BFS function may be incomplete or "
        "buggy.");
  } else {
    bool allGood = true;
    ...
    if (path.front() != maze_start) {
      ...
      errorReaction(
          ...
          "set predecessors correctly with pred[].\n (And make sure you don't "
          "have it backwards.)");
    }
    ...
    if (!GridGraph::allowVerboseTextDescription && !forceDescribePath) {
      std::cout << "Omitting verbose text description of path. You can change "
                   "this in main.cpp"
                << std::endl
    ...
    while (!working_path.empty()) {
      ...
      if (!graph.hasEdge(p1, p2)) {
        ...
        errorReaction(
            "The reported path edge doesn't exist in the original graph");
      } else {
        // When plotting is not allowed, we take special care to print the
        // verbose details of the path in path order.
        if (forceDescribePath || (!GridGraph::allowPlotting &&
                                  GridGraph::allowVerboseTextDescription)) {
          std::cout << "[(" << p1.first << "," << p1.second << ") to ("
                    << p2.first << "," << p2.second << ")] ";
        }
      }
      // step forward
...
void graphBFS_Tester_NoPath() {
  ...
  if (path.empty()) {
    ...
    std::cout
        << "Error: Your graphBFS returned some path when none should exist.\n"
...
void graphBFS_Tester_StartAtGoal() {
  ...
  if (path.empty()) {
    errorReaction(
        "When starting at the goal, graphBFS should return a path\ncontaining "
        "only that one point, but not an empty path.");
  } else {
    ...
    if (allGood)
      std::cout << "Good! When start==goal, the shortest path is to stay at "
                   "the start."
                << std::endl;
    else
      std::cout << "Some warnings or errors occurred in this test."
...
void puzzleBFS_Tester() {
  ...
  std::cout << "This puzzle can be solved from the given starting position, so "
               "puzzleBFS"
            << std::endl
  ...
  // Print a compact string version of the state as well as the plot diagram (if
  // allowed):
  ...
  if (path.empty()) {
    ...
    std::cout << "Returned some path (list of sequential states). Validating..."
              << std::endl
              << std::endl;
...
void puzzleBFS_Tester_NoPath() {
  ...
  std::cout << "(Please allow at least 10 seconds for this test to complete. "
               "If this is"
            << std::endl
            << " inconvenient, you can disable this test in the informalTests "
               "function"
            << std::endl
  ...
  if (path.empty()) {
    ...
    std::cout
        << "Error: Your puzzleBFS returned some path when none should exist.\n"
...
void puzzleBFS_Tester_StartAtGoal() {
  ...
  if (path.empty()) {
    errorReaction(
        "Returned an empty path, but should have returned a path containing "
        "only start.");
  } else {
    std::cout << "Returned some path (list of sequential states). Validating..."
              << std::endl
