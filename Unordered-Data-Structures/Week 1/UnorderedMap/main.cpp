...
void wordcountTests() {
  ...
  if (!wordcount_map.count("bandersnatch") ||
    ...
    std::cout << "Looks like makeWordCounts isn't implemented yet.\nSkipping "
                 "lookupWithFallback tests based upon it."
              << std::endl
              << std::endl;
    return;
  }
  ...
  std::cout << "(Now, note that the Cheshire Cat does not appear in this book.)"
            << std::endl;
  ...
  if (wordcount_map.count("cheshire"))
    ...
    std::cout << "Good, \"cheshire\" was not found, as expected." << std::endl;
  int cheshire_count = lookupWithFallback(wordcount_map, "cheshire", -1);
  std::cout
      << "Looking up \"cheshire\" with fallback of -1. Result (expecting -1): "
      << cheshire_count << std::endl;
  if (cheshire_count != -1)
    errorReaction(
        "Should not have found \"cheshire\" in map, and should have reported "
        "-1 as fallback.");
  else
    std::cout << "Good!" << std::endl;
  std::cout << "Checking that wordcount_map was not modified by the lookup:"
...
void lookupWithFallbackTests() {
...

  {
    ...
    std::cout << "Testing lookupWithFallback with a specially constructed map:"
              << std::endl;
...

  {
    ...
    if (cheshire_count != -1)
      errorReaction(
          "Should hould have reported -1 as fallback\n after not finding "
          "\"cheshire\" in the map.");
    else
      std::cout << "Good!" << std::endl;
    std::cout << "Checking that wordcount_map was not modified by the lookup:"
...
void hashingDemo() {
  {
    ...
    std::cout << "Notice that changing the string by a single character will "
                 "significantly\n change its hash:"
              << std::endl
              << std::endl;
...

    {
      std::string str = "jabberwocky";
      std::cout << "String: \"" << str << "\"" << std::endl;
      // Here is an example of more advanced C++ syntax you might see in the
      // future. In the follow line, the first apperance of "()" syntax only
      // constructs a temporary, anonymous instance of std::hash<std::string>.
  ...
  {
    std::cout << "======= Demonstrating our IntPair hasher =======" << std::endl
              << std::endl;

    std::cout << "Note:\n You don't need to manually invoke our custom hasher "
...
void palindromeTests() {
...

  {
    std::cout << "======= Finding the longest palindrome substring (brute "
              ...
              << std::endl
              << std::endl;
    std::cout << "Note: This may take a few seconds. The duration scales up "
    ...
    try {
      pal_result = longestPalindromeLength(str, 0, str.length() - 1, start_time,
                                           max_duration);
      stop_time = getTimeNow();
      intended_pal_result = pal_result;
      have_intended_result = true;
      std::cout << "Length of longest palindrome substring: " << pal_result
                << std::endl;
    } catch (TooSlowException& e) {
      stop_time = getTimeNow();
      std::cout << "(Timeout!) Brute force is taking too long on this example. "
                   "Giving up."
                << std::endl;
    }
...

  {
    std::cout
        ...
        << std::endl;
    std::cout
        << "Note: If you haven't finished correctly implementing this function "
           "yet,"
        << std::endl
        << " the provided starter code may take even longer than brute force "
           "and time out."
        << std::endl
    ...
    if (timed_out) {
      ...
      if (have_intended_result) {
        if (pal_result != intended_pal_result)
          errorReaction(
              "Your memoized palindrome function returned the wrong result.");
        else
          std::cout << " (Correct!)" << std::endl;
      }
      std::cout << "Calculation time (ms): " << memoized_time << std::endl;
      double speedup = slow_time / memoized_time;
      std::cout << "Speedup over brute force: " << speedup << " times faster"
                << std::endl;
      if (speedup < 2.0)
        errorReaction(
            "Should have been at least 2 times faster.\n (A correct memoized "
            "solution will probably even be MUCH faster than that.)");
      else
        std::cout << " (Good!)" << std::endl;
      ...
      if (found_palindrome != correct_palindrome_substring)
        errorReaction(
            "The substring found is incorrect, so the memoization table\n is "
