...
static void treeFactory(GenericTree<int>& tree) {
  ...
  currentNode->addChild(8);
  currentNode->addChild(15);
  currentNode = currentNode->childrenPtrs[0];
  currentNode->addChild(16);
  currentNode->addChild(23);
  currentNode = currentNode->childrenPtrs[0];
  currentNode->addChild(42);
}
...
int countNullChildrenIterative(N* subtreeRoot) {
  ...
  while (!nodesToExplore.empty()) {
    // Make a copy of the top pointer on the stack, then pop it to decrease the
    // stack
    ...
    if (!topNode) {
      ...
      // Just increment the null counter, then "continue" to jump back to the
      // top of the loop.
      nullChildrenSum++;
      continue;
    }
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while (!nodeQ.empty()) {
    ...
    if (nodeQ.front()) results.push_back(nodeQ.front()->data);
    nodeQ.pop();
  }
